...
public class NioServer {
    public static void main(String[] args) throws IOException {
        ...
        //1、创建服务端通道、选择器与字节缓冲区
        ServerSocketChannel ssc = ServerSocketChannel.open(); //打开一个ServerSocketChannel通道
        Selector selector = Selector.open();//创建一个选择器
        ByteBuffer buffer = ByteBuffer.allocate(1024);//走的是堆内存缓冲区
        //2、为服务端绑定IP地址+端口
        ssc.bind(new InetSocketAddress("127.0.0.1", 8888));
        //3、将服务端设置为非阻塞模式，同时绑定接受事件注册到选择器
        ssc.configureBlocking(false);
        ssc.register(selector, SelectionKey.OP_ACCEPT);
        //4、通过选择器轮询查询出所有已就绪的通道
        while(selector.select() > 0) {
            ...
            while(iterator.hasNext()) {
                ...
                if(next.isAcceptable()) {
                    ...
                    //8、将获取到的客户端连接置为非租塞模式，绑定事件并注册到选择器上
                ...
                //9、如果是读取事件就绪，则先获取对应的通道连接
                    ...
                    //10、从对应的通道中，将数据读取到缓冲区并输出
                    int len = -1;
                    while((len = channel.read(buffer)) > 0) {
                        buffer.flip();//切换到写
                        System.out.println("收到信息：" + new String(buffer.array(), 0, buffer.remaining()));
                    }
                    buffer.clear();
                }
            }
